...
def bcc_Ti_3um_3e3():
    ...
    state = {
        ...
        "a": 5.0,#晶格常数，单位为米（m），表示晶体结构中原子之间的距离。
        "maxseg": 200.0,
        "minseg": 15.2,
        "rtol": 0.25,#相对误差容限，控制数值计算的精度。收紧到 0.1（原 0.5 太宽，导致外层 dt 每步 ×1.2 暴涨）
        "rann": 0.5,#邻近距离，单位为米（m），用于判断位错线段之间是否相互作用。
        "nextdt": 1e-12,
        "maxdt": 1e-7,#外层时间步上限。原 1e-7 配合 vmax=3400 m/s 子循环数爆炸，单步 ~5h；压到 1e-9，每外层步应变增量 3e-6 仍够粗。
    ...
    timeint   = TimeIntegration(integrator='Subcycling', rgroups=[0.0, 10.0, 60.0, 200.0], state=state, force=calforce, mobility=mobility)#时间积分器，使用子循环方法（Subcycling）来处理不同时间尺度的位错运动。rgroups 定义了不同时间段的分组，用于调整时间步长以适应不同阶段的模拟需求。
