...
def bcc_Ti_3um_3e3():
    ...
    state = {
        ...
        "a": 1.0,#位错核心半径（core radius），单位为伯氏矢量模长 b，1.0·b ≈ 0.28 nm。用于非奇异力计算，并非晶格常数。
        "maxseg": 480.0,#位错线段最大长度，单位为 b，480·b ≈ 134 nm。
        "minseg": 120.0,#位错线段最小长度，单位为 b，120·b ≈ 33 nm。
        "rtol": 0.25,#相对误差容限，控制数值计算的精度。收紧到 0.1（原 0.5 太宽，导致外层 dt 每步 ×1.2 暴涨）
        "rann": 0.5,#碰撞/邻近判定距离，单位为 b，0.5·b ≈ 0.14 nm，用于判断位错线段之间是否相互作用。
        "nextdt": 1e-12,
        "maxdt": 1e-6,#外层时间步上限。原 1e-7 配合 vmax=3400 m/s 子循环数爆炸，单步 ~5h；压到 1e-9，每外层步应变增量 3e-6 仍够粗。
    ...
    calforce  = CalForce(force_mode='SUBCYCLING_MODEL', state=state, Ngrid=128, cell=net.cell)#3*3*3 的网格用于计算位错之间的相互作用力。
    ...
    timeint   = TimeIntegration(integrator='Subcycling', rgroups=[0.0, 200.0, 800.0, 2000.0], state=state, force=calforce, mobility=mobility)#时间积分器，使用子循环方法（Subcycling）来处理不同时间尺度的位错运动。rgroups 定义了不同时间段的分组，用于调整时间步长以适应不同阶段的模拟需求。
    ...
    sim = SimulateNetworkPerf(calforce=calforce, mobility=mobility, timeint=timeint,
                              ...
                              write_freq=1, write_dir=output_dir, restart=restart)#每 10 步保存一次数据
    sim.run(net, state)
