...
def bcc_Ti_3um_3e3():
    ...
    timeint   = TimeIntegration(integrator='Subcycling', rgroups=[0.0, 200.0, 800.0, 2000.0], state=state, force=calforce, mobility=mobility)#时间积分器，使用子循环方法（Subcycling）来处理不同时间尺度的位错运动。rgroups 定义了不同时间段的分组，用于调整时间步长以适应不同阶段的模拟需求。 rgroups 把内组阈值放宽到 [0, 200, 800, 2000] Force time 是否真减小，不能盲目放宽
