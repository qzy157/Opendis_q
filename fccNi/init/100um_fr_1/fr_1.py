...
def fcc_Ni_100um_frank_read():
    ...
    Lbox = 100.0e-6 / state["burgmag"]  # 模拟盒子边长 (b)，100 um
    ...
    # 臂长与 10 um 相同：FR 开动应力 ~ mu*b/L 只由臂长决定，保证不同盒子尺寸可比
    ...
    # 12 个滑移系轮流分配后打乱，各滑移系源数相差不超过 1（N_dis=767 时每个滑移系 63~64 个源）
    ...
    write_vtk(net, os.path.join(script_dir, 'fcc_Ni_100um_1e10_frank_read_1.vtk'), crystal='FCC')
    write_data(net, os.path.join(script_dir, 'fcc_Ni_100um_1e10_frank_read_1.data'))
    pyexadis.finalize()


if __name__ == "__main__":
    fcc_Ni_100um_frank_read()
