...
def force_100_percent(cs_p):
    ...
    可以干净地检验 thermal 的链构型 bug —— 本文件正是有 bug 的 thermal 版本。
    """
    # Bulk
...
def make_cross_slip_fcc_thermal(state, force_module, temperature=300.0):
    ...
    本文件是 100% 版本：在参考 yes.py 的基础上把热激活势垒清零，
    使得能交滑移的位错必然发生交滑移（见 force_100_percent）。
    thermal 版保留旧的"整臂弦向判螺型 + 弯臂整条丢弃"链构型 bug，
    在 100% 触发下可与 test / wansheng 直接对比出该 bug 的影响。
    """
...
def example_fcc_Cu_15um_1e3_with_cross_slip():
    ...
    state = {
        ...
        "nextdt": 1e-10,
        "maxdt": 1e-9,
        "use_glide_planes": True,   # CrossSlipFCCThermal::handle() 要求 use_glide_planes=true（与 test 配置对齐）
    }

    output_dir = 'output_cross_slip_thermal_100'
