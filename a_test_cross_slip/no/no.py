...
def example_fcc_Cu_15um_1e3_no_cross_slip():
    """example_fcc_Cu_15um_1e3_no_cross_slip:
    Strain hardening simulation of fcc Cu (15 um box, strain rate 1e3/s)
    WITHOUT cross-slip. Used as the baseline case for comparison against
    the thermally-activated ParaDiS cross-slip model in:
        test_with_cross_slip_exadis.py

    Matches the setup of examples/10_strain_hardening identically — same
    initial configuration, force model, mobility, integrator, collision,
    topology, and remesh — but cross_slip is explicitly omitted.

    Output is written to output_no_cross_slip/ so results from both runs
    can be plotted side-by-side with plot_compare.py.
    """
    ...
    output_dir = 'output_no_cross_slip'
    ...
    # cross_slip is intentionally omitted
