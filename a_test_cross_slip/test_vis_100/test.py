...
def make_cross_slip_fcc_test(state, force_module, temperature=300.0):
    ...
    本文件是 100% 版本：在参考 yes_test.py 的基础上把热激活势垒清零，
    ...
    # minRunLength: 螺型 run 最小长度（b）。与参考 yes_test.py 保持一致（1200 b），
    # 使 test/thermal/wansheng 的对比只在链构型算法上有别、几何过滤强度可比。
...
def example_fcc_Cu_15um_1e3_with_cross_slip():
    ...
    output_dir = 'output_cross_slip_test_100'
